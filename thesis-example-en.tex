...
\documentclass[
  ...
  DIV=none, % Disable KOMA-Script margin calculation to use geometry instead
  headings=small,
  bibliography=totoc,
...
% \documentclass[
%   fontsize=10pt, % FOR A5
%   % fontsize=12pt, % FOR A4
%   % fontsize=12pt, % FOR A4
%   numbers=noenddot,
%   english,  % English as main language; this parameter is passed to other packages (e.g., selnolig in the case of lualatex)
%   paper=a5,
...
\typearea[current]{last} % For A5
% Use geometry to strictly enforce the "at least 2cm" margin rule
\usepackage[a4paper, left=3cm, right=3cm, top=3cm, bottom=6cm, bindingoffset=5mm, footskip=3cm]{geometry}
% \usepackage[a4paper, left=3cm, right=2.5cm, top=2.5cm, bottom=5cm, bindingoffset=5mm, footskip=2.5cm]{geometry}
\usepackage[
...
\usepackage{diagbox}
\newsavebox{\tempcaptionbox} % Define this in your preamble
\ifluatex
  \usepackage{spelling}
  \spellingoutput{off}
\fi
...
\usepackage[
  ...
  maxbibnames = 6, % List all authors in bibliography
...
\DeclareSIUnit{\dummy}{}
\DeclareSIUnit{\sqrthertz}{\text{$\sqrt{\mathsf{Hz}}$}}
...
% Load the official multicolumn glossary styles
\usepackage{glossary-mcols} 
\makenoidxglossaries
% Set the style to the native multicolumn index
...
\examchair{Prof.~Dr.~Mathias Scheurer}
\dateofexamination{3\textsuperscript{rd} August, 2026}
\placeofbirth{Cochin, India}
\faculty{Fakultät 8 für Mathematik und Physik}
...
% \layout
\onehalfspacing
\frontmatter %is removed to keep numbering continuous and Arabic from the title page

\include{Chapters/abstract_de.tex}
\cleardoublepage 
...
\cleardoublepage
{
\footnotesize % Reduces the font size of the entries
\printnoidxglossary[
    type=abbreviations, 
    sort=letter, 
    title={Abbreviations} % Works normally now without shifting
]
}
%%%%%%%%%%%%%%%%%
\cleardoublepage
...
\pagestyle{scrheadings}
\mainmatter %is removed to prevent the page counter from resetting to 1
...
\input{Chapters/ai_tools.tex}
\cleardoublepage
\input{Chapters/conformity.tex}
\cleardoublepage
